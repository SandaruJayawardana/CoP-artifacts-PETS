...
def get_CMF(filtered_original_data, filtered_perturb_data, Z_alphabet, X_k_alphabet):
    ...
    for index_i, i in enumerate(filtered_perturb_data[:,:]):
...
def get_cmf_pmf_dict_with_alphabet(data, is_pmf = False):
    ...
    try:
        ...
        if is_pmf:
            with open(f'{FILE_LOCATION}/dummy_pmf.pkl', 'rb') as file:
                PMF_dict, _, alphabet_dict = pickle.load(file)
                validate(PMF_dict, COLUMNS)  
        else:  
            with open(f'{FILE_LOCATION}/dummy_cmf.pkl', 'rb') as file:
        ...
        for index_i, i in enumerate(COLUMNS[:]):
            ...
            for index_j, j in enumerate(COLUMNS):
                ...
                if key_ not in CMF_dict.keys():
                    CMF, PMF = get_CMF(filtered_original_data, filtered_perturb_data, Z_alphabet, X_k_alphabet) 
                    CMF_dict[key_] = CMF
                    PMF_dict[key_] = PMF
            alphabet_dict[i] = X_k_alphabet
            with open(f'{FILE_LOCATION}/dummy_cmf.pkl', 'wb') as file:
                pickle.dump((CMF_dict, COLUMNS, alphabet_dict), file)
            with open(f'{FILE_LOCATION}/dummy_pmf.pkl', 'wb') as file:
