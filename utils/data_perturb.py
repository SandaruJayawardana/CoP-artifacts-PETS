...
def data_generator(parameter):
    data_values, eps, repetitions, mechanism = parameter
    output_list = []
...
class Perturbation():
    ...
    def perturb(self, EPS_ARRAY, NUM_REPITTIONS):
        with ProcessPoolExecutor(max_workers=10) as executor:
            futures = [executor.submit(data_generator, (self.data[self.COLUMNS].values, EPS_ARRAY[eps], NUM_REPITTIONS, self.mechanism)) for eps in range(len(EPS_ARRAY))]
